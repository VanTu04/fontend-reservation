...
const ManageAllOrder = () => {
  ...
  const exportOrderToExcel = (order) => {
    ...
    const sheetData = [
      ...
      [], // Dòng trống
      ["Thời Gian Đặt:", new Date(order.orderTime).toLocaleString()],
    ...
    order.orderItems.forEach((item) => {
      // Kiểm tra giá trị price có phải là số hợp lệ không
      let priceFormatted = item.food.price;
      if (
        isNaN(priceFormatted) ||
        priceFormatted === null ||
        priceFormatted === undefined
      ) {
        priceFormatted = "0"; // Nếu không hợp lệ, gán giá trị mặc định
      } else {
        priceFormatted = new Big(priceFormatted).toLocaleString(); // Đảm bảo xử lý kiểu BigDecimal
      }

      sheetData.push([item.food.name, item.quantity, `${priceFormatted} VND`]);
    });
